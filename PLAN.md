...
**§8 steps 3 and 4 done — full flow working end-to-end.** Retell single-prompt agent (`Tony's Pizza — order taker`, `agent_04be53…11d`) built from the spec in [`retell/`](retell/README.md) — system prompt with baked-in menu ([`retell/agent-prompt.md`](retell/agent-prompt.md)) + the `create_order` custom function ([`retell/create_order.tool.json`](retell/create_order.tool.json)) with **"Payload: args only"** ON + `X-Api-Key` header, pointed at `https://34-244-32-152.sslip.io/api/orders`. Begin Message set and agent published so it greets first on a real call. **Verified:** simulator and a real inbound phone call both round-trip → `201` + confirmation number, row lands in the live EC2 `database.sqlite`.

**All four build-order steps complete.** Remaining work is optional — see §7 stretch goals.
...
**Done — agent live and taking real calls.** Spec + step-by-step in [`retell/README.md`](retell/README.md).
...
- [x] Attach a test phone number to place a real call. — number attached, agent published with a Begin Message so it greets first; a real inbound call round-trips a full order to the live DB.
...
3. ~~Build the Retell agent + `create_order` tool pointed at `https://34-244-32-152.sslip.io/api/orders`; test in the simulator.~~ **Done.** Spec under [`retell/`](retell/README.md).
4. ~~Place one real test call end-to-end, verify the row lands in the SQLite file.~~ **Done.** Real inbound call round-trips a full order to the live DB.
